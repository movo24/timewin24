...
- **DEBT-023 (✅ largement traité)** — tests ajoutés : RBAC (16), coût employeur+Decimal (7), utilitaires purs geo/shift/timeline (13). Suite 95→187 (+38 contraintes dures [constraints.ts complet] + 18 scoring solveur). Reste optionnel : scénarios/suggestions solveur. → M116.
...
- **DEBT-032 (🔄 en cours)** — lint : `no-unused-vars` (81), `prefer-const` (2), `no-unescaped-entities` (9) **résorbés** (total eslint 178→85), config eslint durcie (`ignoreRestSiblings` pour ne pas casser les strips de secrets, `argsIgnorePattern '^_'`). Reste **délibérément non touché** (runtime-sensible) : `any` (39), `set-state-in-effect` (30), `no-img-element` (8), hooks (3). Note : `(prisma as any)` dans `pos-feed/store-schedules` suggère un drift de typage Prisma à investiguer. → M142.
