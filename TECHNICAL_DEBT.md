...
- **DEBT-032 (🔄 en cours)** — lint : `no-unused-vars` (81), `prefer-const` (2), `no-unescaped-entities` (9) **résorbés** (total eslint 178→85), config eslint durcie (`ignoreRestSiblings` pour ne pas casser les strips de secrets, `argsIgnorePattern '^_'`). `any` réduit **39→29** (callbacks `.map/.filter` result-mapping retypés par inférence + 8 directives eslint-disable orphelines retirées ; total lint 85→72). Reste : `where: any` Prisma (pattern dynamique défendable — types stricts révèlent surtout des casts string→enum à faible valeur), `set-state-in-effect` (30), `no-img-element` (8), hooks (3). ✅ `(prisma as any)` dans `pos-feed/store-schedules` **résolu** : artefact inutile (storeSchedule typecheck sans cast, comme dans 4 autres fichiers) — retiré, type-checking restauré sur findMany/upsert. → M142.
