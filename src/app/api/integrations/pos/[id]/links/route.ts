...
export async function DELETE(
  ...
  try {
    ...
    if (type === "store") {
      // Vérifier que le lien appartient bien à ce provider
      const link = await prisma.posStoreLink.findUnique({ where: { id: linkId } });
      if (!link) return errorResponse("Lien introuvable", 404);
      if (link.providerId !== id) return errorResponse("Ce lien n'appartient pas à ce provider", 403);
      await prisma.posStoreLink.delete({ where: { id: linkId } });
    } else if (type === "employee") {
      const link = await prisma.posEmployeeLink.findUnique({ where: { id: linkId } });
      if (!link) return errorResponse("Lien introuvable", 404);
      if (link.providerId !== id) return errorResponse("Ce lien n'appartient pas à ce provider", 403);
